...
- DuckLake data files backing published analytical tables
- dataset manifests and contracts where useful
- MLflow artifacts
- Airflow logs
...
NFS server IP and export root are configured in
`ansible/inventory/group_vars/k3s_servers.yml`. The PV paths in `k8s/nfs-pv.yml` must
use subdirectories under that export root.
...
- Airflow values with external PostgreSQL metadata and git-sync DAG delivery
...
Deploy the current monitoring stack with:
...
Deploy Airflow separately once the Airflow app and database secrets are available:

```bash
cp k8s/.airflow-app.env.example k8s/.airflow-app.env
# k8s/.airflow-db.env is generated by OpenTofu, or provide connection= manually
make deploy-airflow
```

`make deploy-all` attempts Airflow, MLflow if `helm/mlflow.yml` exists, and monitoring.
