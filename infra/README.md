...
```bash
cp .airflow-app.env.example .airflow-app.env
cp .ml-app.env.example .ml-app.env
cp .monitoring-app.env.example .monitoring-app.env
...
- `.grafana-admin.env` is generated by OpenTofu and used to create
  `grafana-admin-credentials` in the `monitoring` namespace.
- `.airflow-app.env` is user-managed, gitignored, and maps directly to
  `airflow-app-secrets` in `airflow`.
- `.ml-app.env` is user-managed, gitignored, and maps directly to
  `ml-app-secrets` in `ml`.
- `.monitoring-app.env` is user-managed, gitignored, and maps directly to
...
| `.airflow-app.env` | Manual Airflow application secrets |
| `.ml-app.env` | Manual ML application secrets |
| `.monitoring-app.env` | Manual monitoring application secrets |
